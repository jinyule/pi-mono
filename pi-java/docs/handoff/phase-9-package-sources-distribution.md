...
- Java package installs now also reuse explicit `.npmrc` auth and npm config directly, so project/global installs can carry scoped or default registry credentials, keep extra npm settings like `strict-ssl`, and let project `.npmrc` auth win over saved host tokens when both exist.
...
- The next slice should either move from installer preflight into actual Windows installer packaging once WiX is available locally, or decide whether auth work should keep growing past `.npmrc` + token-import flows into fuller browser-driven login/config UX.
