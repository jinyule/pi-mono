...
- Phase 8 (behavior parity): in progress.
  ...
  - Interactive slash-command parity now also includes `/session` session-info output with real message/token/cost totals, `/hotkeys` keyboard-shortcut output sourced from the configured editor/app bindings, and `/changelog` changelog rendering from the nearest available coding-agent changelog file, all rendered as persistent transcript panels instead of transient status text.
