...
3. 追平 footer token/cost/model 信息。（已完成前六刀：single-line usage/cost/model footer；footer ANSI hierarchy + reasoning summary；provider-aware model summary first cut；context window indicator first cut；narrow-width truncation first cut；auto-compaction suffix first cut）
