...
- 阶段 8：已开始，已完成 session selector current/all scope toggle 首版（resolver 提供 `current/all` 双列表、picker 支持 `Tab` 切 scope）、sort toggle 首版（`Ctrl+S` 在 `recent/relevance` 间切换，relevance 基于 label/file/cwd/path token 命中排序）、named-only filter 首版（`Ctrl+N` 在 `all/named` 间切换，blank name 视为 unnamed）、path show/hide toggle 首版（`Ctrl+P` 切 path 描述显示）、threaded sort 首版（默认从 `threaded` 起步，`Ctrl+S` 第三态进入 `threaded`，空 query 时按 parentSessionPath 树状展开）、fuzzy parser/search 首版（quoted phrase、`re:` regex、subsequence fuzzy，搜索文本覆盖 `id/name/allMessagesText/cwd/path`）、keybindings.json loader 首版（CLI 启动时从 `~/.pi/agent/keybindings.json` 读取 session selector 相关键位覆盖）、session selector 单行顶栏宽度感知截断首版（窄终端下保留 scope summary，并对 title/status 做安全截断）、session selector metadata 响应式列宽首版（中等宽度下仍保留 `msg/age` 等描述列，避免过早退化成纯 label）、session selector 顶部 info/hint 行显式换行首版（窄终端下按词换行，不再依赖硬换列打断提示 token）、session selector ANSI 样式层级首版（selected row 用 accent/bold，metadata/scroll/no-match 用 muted）、session selector 顶部 status/error/info ANSI 首版（header title 用 bold、summary/hints 用 muted、delete confirm 用 warning、load error 用 error）、session selector summary 分段着色首版（active scope / loading progress 用 accent，inactive scope 分隔段保持 muted）、tree/fork selector 复用 ANSI 层级首版（overlay title 用 bold，hint 用 muted，list row 复用 session selector theme）、session selector regex parse error 首版（search 区上方显示 `Invalid regex`，并走 error 样式）、session selector empty/no-match/error 状态分层首版（loading/load-error 时不再退化成通用 no-match，blank query/no-result/invalid regex 分别显示 session-specific 状态文案）、session selector empty-state hint parity 首版（current scope empty / named-only empty 时补 `tab` 和 named-toggle 恢复提示，文案对齐 TS 行为）、selector metadata 右侧对齐首版（`SelectList` 会从实际 label 宽度回收空余列给 description，保留右侧 metadata，并留安全边距避免 terminal wrap）、interactive footer token/cost/model 首版（14 行以上终端显示一行 footer，累计 assistant usage 并展示 `↑/↓/R/W/$` 与当前 model / thinking 信息）、interactive footer context window indicator 首版（按最近 assistant usage 的 `totalTokens/contextWindow` 追加 `x%/window` 指标，并在高占用时切到 warning/error 样式）、interactive footer narrow-width truncation 首版（宽度不足时优先保留右侧 model 摘要，并把左侧 usage/context 指标截断到剩余空间）、interactive footer auto-compaction suffix 首版（根据 `/compaction/enabled` 在 context indicator 后追加 `(auto)`，并允许 session 级开关关闭）、interactive footer post-compaction unknown usage 首版（最近一次 compaction 后如果还没有新的 assistant usage，则把 context indicator 切到 `?/%window`，避免继续显示 stale pre-compaction 百分比）、interactive footer idle context baseline 首版（没有 assistant usage 时显示 `0.0%/%window`，并改成紧凑单行布局，避免宽终端 padding + ANSI 把右侧 model 摘要挤爆）、interactive footer provider-count gating 首版（只有多 provider 可用时才显示 `provider/` 前缀，单 provider 场景保持只显示 model id）、interactive footer cwd/session line 首版（15 行以上终端追加第二行 footer，显示 `cwd • session name`，并优先保留第一行 stats，避免关键统计被裁掉）、interactive footer cwd/session middle-truncation 首版（长 `cwd • session name` 现在走中间截断，优先保留路径起止和 session name，贴近 TS footer copy）、interactive footer git-branch 首版（footer 第二行会从 session cwd 解析 `.git/HEAD`，显示 `cwd (branch) • session name`，兼容普通 repo 和 worktree）、interactive footer git-branch watcher 首版（interactive mode 启动时监听 git HEAD 所在目录，branch 切换后自动重绘 footer）、interactive app keybinding `cycleModelBackward` 首版（新增反向 model cycle、`shift+ctrl+p` 默认键位、keybindings loader alias、interactive status 回显与 session 持久化）、interactive app keybinding `newSession` 首版（新增空白新 session 切换、keybindings loader alias、interactive 状态回显与后续 prompt sessionId 接线）、interactive `/settings` selector 首版（搜索式 settings overlay、`auto-compact`/`steering mode`/`follow-up mode`/`thinking level` 修改、当前 session + global settings 同步、`PiSdkSession` 启动时回放默认 thinking 和 queue modes）、interactive model selector richer metadata 首版（当前项优先排序、row label 改成 model id、right metadata/selected detail 补 `provider`/`model name`/`thinking`/`context window`）、interactive model selector `all/scoped` dual-scope 首版（有 scoped models 时支持 `Tab` 切 `All/Scoped`，并把 selector 选择结果扩到 registry all-model set；model select / cycle 同步保存 `defaultProvider/defaultModel`）、startup model resolution saved-defaults 首版（CLI 启动时会回读 settings 里的 `defaultProvider/defaultModel`，用于恢复上次 model selector / cycle 保存的默认模型）、interactive transport setting 首版（`/settings` 新增 `transport` 项、`PiSdkSession` 启动时回放 `transport` 默认值、session 内更新会立即影响后续 prompt 的 stream options）、interactive model selector custom search/ranking 首版（provider/modelId/modelName/thinking 字段自定义搜索、exact/prefix/boundary 优先级排序、token-level fuzzy fallback、`No matching models` 文案）、interactive model selector selected-detail 首版（overlay 顶部显示当前选中 `provider/model` 与 metadata 摘要，方向键移动时 detail 会同步刷新）、interactive pending queue steering parity 首版（streaming 中普通回车改为 queue steering message，status 会合并显示 `Steering:` / `Follow-up:`，`alt+up` 恢复顺序改成 steering 在前、follow-up 在后）、interactive model selector bottom detail panel 首版（all-scope 顶部补“仅显示已配置 API key 的模型”提示，selected detail 改成列表下方的独立面板，更接近 TS 的 detail 区布局）、interactive model selector provider/detail split 首版（列表 metadata 收窄成 provider badge，model name/thinking/context 下沉到底部 detail panel，进一步接近 TS 的 row/detail 分工）、interactive model selector detail panel container 首版（selected detail panel 补上下 separator，形成更接近 TS 的独立容器层级）、interactive model selector outer border 首版（整个 selector 补顶部/底部 separator，接近 TS `DynamicBorder` 外框层级）、interactive model selector scope/hint copy 首版（`Scope: all | scoped` 与 `tab scope (all/scoped)` 文案对齐 TS）、interactive model selector current-checkmark ANSI 首版（当前模型的 `✓` 改成 success 样式，靠近 TS 的当前项标记层级）、interactive model selector detail ANSI hierarchy 首版（detail panel 标题改成 accent+bold，`provider/model` 行改成 provider muted + model bold，继续收紧 TS 层级差异）、interactive selected-row metadata channel 首版（`SelectListTheme` 新增 `selectedDescription()`，选中行 description 不再被硬包进 `selectedText()`，model selector 的 provider badge 现在能稳定走 muted 样式）、interactive model selector dedicated row theme 首版（model selector 不再复用 session selector theme，选中行主文本改成 accent+bold，继续靠近 TS 的选中模型高亮）、interactive model selector keybinding-aware hints 首版（scope/select/cancel 提示改成读取 `EditorKeybindings.global()`，不再硬编码 `tab/enter/esc` 文案）、interactive model selector detail field hierarchy 首版（detail field 的 label/value 拆开着色，label bold、value muted，继续压缩 TS 视觉差异）、interactive model selector wrapped hints/warning 首版（warning 与 hint 行改成按终端宽度换行，窄终端下不再横向溢出）、interactive settings selector keybinding-aware hints 首版（顶部说明和底部 search/change/cancel hint 改成读取 `EditorKeybindings.global()`，同时保留 space 作为固定 secondary activate key）、interactive settings selector hide-thinking parity 首版（新增 Hide thinking setting、回写 hideThinkingBlock，并让 interactive transcript 真正隐藏 thinking block）、interactive settings selector quiet-startup parity 首版（新增 Quiet startup setting、回写 quietStartup，并让 interactive header 真正静默）、interactive app keybinding toggleThinking 首版（新增 ctrl+t 默认键位、keybindings loader alias、interactive transcript/status 直接复用 hide-thinking setting 切换）、interactive app keybinding clear 首版（新增 ctrl+c 默认键位、keybindings loader alias、interactive clear once / exit twice 语义）、interactive app keybinding exit 首版（新增 ctrl+d 默认键位、keybindings loader alias、仅在空输入时触发 interactive exit）、interactive app keybinding expandTools 首版（新增 ctrl+o 默认键位、keybindings loader alias、interactive transcript 支持展开 tool result details）、interactive settings selector editor-padding first cut（新增 `Editor padding` 条目、0-3 值持久化，并把 interactive prompt 的 horizontal padding 接到 `Input` 渲染）、interactive runtime dark/light theme first cut（`PiCliAnsi` 新增 light/dark palette，interactive `renderState()` 会跟随 theme setting 重新着色 ANSI 输出）、interactive settings selector theme submenu preview first cut（`Theme` 条目改成 submenu，方向键移动时走 runtime ANSI preview，`Esc` 恢复原 theme，`Enter` 提交选中 theme）、interactive settings selector thinking submenu first cut（`Thinking level` 条目改成 submenu，补齐 `off/minimal/low/medium/high/xhigh` 描述文案，`Enter` 提交选中 level）、interactive settings selector hardware-cursor/clear-on-shrink first cut（新增 `Show hardware cursor` / `Clear on shrink` 条目、settings 持久化，并把值接到 `Tui` / `DiffRenderer` 的 runtime render 开关）、interactive settings selector TS-style copy first cut（`steering mode` / `follow-up mode` / `transport` / `quiet-startup` 描述文案对齐 TS，`transport` 值顺序改成 `sse/websocket/auto`）。, interactive custom theme loader first cut (startup loads JSON themes from ~/.pi/agent/themes, .pi/themes, and explicit --theme paths; /settings -> Theme shows custom names), interactive custom theme hot reload first cut (/reload rescans theme paths, refreshes the PiCliAnsi registry, and surfaces theme warnings), interactive startup loaded-resources listing first cut (shows context/extensions/custom themes when quietStartup=false, and keeps quiet reloads limited to diagnostics-only warnings), interactive custom theme core tokens first cut (PiCliThemeLoader now resolves optional `dim`/`text`/`border`/`borderMuted` tokens with safe fallbacks, and startup hint text plus model-selector borders now consume theme-specific styling), interactive model selector top-hint parity first cut (drops the extra filter/select/cancel copy so the header matches TS scope-only or warning-only messaging), interactive model selector scope-hint styling first cut (the key segment now uses dim styling while the description stays muted, matching TS `keyHint(...)` layering), interactive tree/fork selector key-hint styling first cut (hardcoded Enter/Esc copy is replaced with configurable dim/muted key hints via `PiCliKeyHints`), interactive settings selector key-hint styling first cut (top search hint and submenu footer now use `PiCliKeyHints` with dim keys and muted descriptions while preserving the fixed `space` secondary action).
- 阶段 9：未开始。
## 阶段 0：项目骨架
...
1. 追平 keybinding 语义与配置格式。（已完成增量：session selector loader/app-editor layering；interactive app actions interrupt/clear/exit/resume/tree/fork/cycleThinkingLevel/cycleModelForward/cycleModelBackward/selectModel/expandTools/pasteImage/toggleThinking/followUp/dequeue/newSession/suspend/externalEditor；`shift+tab` matcher；`shift+ctrl+p` matcher；`ctrl+l` matcher；`alt+up` matcher；`--models` scoped cycle first cut）
2. 追平 session selector、tree selector、model selector、settings selector。（已开始：session selector current/all scope toggle、sort toggle、named/path toggle、threaded/fuzzy、keybindings loader、loading/progress header、TS-style fuzzy scoring、search text scope parity、header/status/hint 文案、app/editor keybinding 分层、path/cwd metadata、single-row title/scope header、single-row 顶栏宽度感知截断、metadata 响应式列宽、顶部 info/hint 行显式换行、ANSI 样式层级、顶部 status/error/info ANSI、summary 分段着色、tree/fork selector 复用 ANSI 层级、regex parse error 状态；interactive app keybindings 已扩到 interrupt/clear/exit/resume/tree/fork/cycleThinkingLevel/cycleModelForward/cycleModelBackward/selectModel/expandTools/pasteImage/toggleThinking/followUp/dequeue/newSession/suspend/externalEditor；`--models` scope resolve + forward cycle 首版；model selector overlay first cut；pending follow-up queue status first cut；`/settings` selector first cut，当前覆盖 `auto-compact`、`steering mode`、`follow-up mode`、`thinking level`；model selector richer metadata first cut，当前补了 current-first 排序和 `provider/name/thinking/context` metadata；model selector `all/scoped` dual-scope first cut，当前支持 registry all-models 和 scoped-models 间切换；detail panel container first cut，当前补了 selected detail panel 的上下 separator；outer border first cut，当前补了 selector 顶部/底部 separator；scope/hint copy first cut，当前补了 `Scope:` 和 `tab scope (all/scoped)` 文案；current-checkmark ANSI first cut，当前补了当前模型 `✓` 的 success 样式；detail ANSI hierarchy first cut，当前补了 detail panel 标题/`provider/model` 的层级；selected-row metadata channel first cut，当前补了 `SelectList.selectedDescription()`；dedicated row theme first cut，当前把 model selector 的选中行主文本切到 accent+bold；keybinding-aware hints first cut，当前把 selector hints 接到 `EditorKeybindings.global()`；detail field hierarchy first cut，当前把 detail field label/value 分开着色；wrapped hints/warning first cut，当前把 warning/hint 行改成按宽度换行；settings selector keybinding-aware hints first cut，当前把顶部说明和底部 search/change/cancel hint 接到 EditorKeybindings.global()，并保留 space 作为固定 secondary activate key；settings selector hide-thinking parity first cut，当前补了 hideThinkingBlock 持久化与 interactive transcript 隐藏渲染；settings selector quiet-startup parity first cut，当前补了 quietStartup 持久化与 interactive header 静默；toggleThinking app keybinding first cut，当前补了 ctrl+t 默认键位、loader alias 和 transcript/status 切换；clear app keybinding first cut，当前补了 ctrl+c 默认键位、loader alias 和 clear once / exit twice 语义；exit app keybinding first cut，当前补了 ctrl+d 默认键位、loader alias 和仅空输入退出语义；expandTools app keybinding first cut，当前补了 ctrl+o 默认键位、loader alias 和 tool result details 展开语义；pasteImage app keybinding first cut，当前补了 Windows `alt+v` / 非 Windows `ctrl+v` 默认键位、loader alias 和 interactive image attachment 提交语义；interactive startup header keybinding hints first cut，当前补了 keybinding-aware startup hint，并仅在较高 terminal 显示；restartable process-terminal lifecycle first cut，当前补了 `ProcessTerminal.stop()/start()` 可重入底座，为后续 `suspend` / `externalEditor` 铺路；suspend app keybinding first cut，当前补了 `ctrl+z` 默认键位、loader alias 和 interactive suspend dispatch，Windows 上显式提示 unsupported，POSIX 上走 stop/start + `TSTP`；externalEditor app keybinding first cut，当前补了 `ctrl+g` 默认键位、loader alias 和 external editor 回写，当前会按 Java 单行 `Input` 约束把多行内容压平成单行；model selector warning/hint copy parity first cut，当前把 scoped hint 拆回 `tab scope (all/scoped)` 独立一行、统一空结果文案为 `No matching models`，并把 detail label 对齐到 `Model Name`；settings selector theme first cut，当前补了 `Theme` 条目、settings 持久化、`dark/light` 可选值、runtime ANSI theme switch，以及 submenu preview / `Esc` restore；settings selector thinking first cut，当前把 `Thinking level` 改成 submenu，并补了各 level 的描述文案；settings selector hardware-cursor/clear-on-shrink first cut，当前补了 `Show hardware cursor` / `Clear on shrink` 条目、settings 持久化，以及 `Tui` / `DiffRenderer` runtime 接线；settings selector TS-style copy first cut，当前把 `steering mode` / `follow-up mode` / `transport` / `quiet-startup` 描述文案对齐 TS，并把 `transport` 值顺序改成 `sse/websocket/auto`; custom theme loader first cut, wiring ~/.pi/agent/themes, .pi/themes, explicit --theme paths, PiCliAnsi custom palette registration, and /settings custom theme listing; custom theme hot reload first cut, wiring theme registry refresh into /reload so ReloadResult carries theme warnings and /settings sees new themes; startup loaded-resources listing first cut, wiring context/extensions/custom themes into the interactive startup surface and keeping quiet-startup reloads diagnostics-only; settings selector double-escape-action first cut，当前补了 `Double-escape action` 条目，并把真实 `Escape` 双击空编辑器的行为接到 `tree/fork/none`；settings selector editor-padding first cut，当前补了 `Editor padding` 条目、`editorPaddingX` 持久化和 interactive prompt horizontal padding 渲染）
