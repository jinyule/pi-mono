...
- Phase 8 (behavior parity): in progress.
  ...
  - Interactive command/status copy now also includes `/name` slash-command wiring, current-name lookup, session-name updates, terminal-title refresh on name changes, and terminal-title reset when `/new` starts a fresh unnamed session.
