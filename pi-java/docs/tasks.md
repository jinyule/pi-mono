...
- 阶段 3：已完成 `SessionHeader / SessionEntry / SessionTreeNode / SessionContext / SessionDocument`、`Session JSONL parse/write skeleton`、`v1 -> v2 -> v3 migration`、`buildSessionContext()` replay contract tests。
- 阶段 3：下一步补 `SessionManager` skeleton。
- 阶段 4 到阶段 9：未开始。
