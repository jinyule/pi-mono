...
- 阶段 8：已开始，已完成 session selector current/all scope toggle 首版（resolver 提供 `current/all` 双列表、picker 支持 `Tab` 切 scope）、sort toggle 首版（`Ctrl+S` 在 `recent/relevance` 间切换，relevance 基于 label/file/cwd/path token 命中排序）。
- 阶段 9：未开始。
