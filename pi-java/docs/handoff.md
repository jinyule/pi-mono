...
- 阶段 0 到阶段 5：已收尾。
- 阶段 6 `pi-tui`：进行中，已完成 core contracts、terminal base support、diff renderer、overlay/cursor、`Container`/`Text`/`TruncatedText`、`Input`、`Editor`、`Markdown`。
- 阶段 7 到阶段 9：未开始。
...
1. `pi-tui`：`Loader`
2. `pi-tui`：`SelectList` / `SettingsList`
