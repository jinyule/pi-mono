...
- 阶段 7 `pi-cli` / `pi-sdk`：已开始，已完成 `pi-cli` CLI 参数解析首版、`PiAgentSession` skeleton、最小 `interactive` mode、`print` mode 首版、`json` mode 首版。
- 阶段 8 到阶段 9：未开始。
...
1. `pi-cli`：`rpc` mode
2. `pi-sdk` facade
3. `pi-cli`：CLI main entry / startup pipeline
