...
1. `pi-cli`：继续补 real module wiring 的 interactive exit 语义
2. `pi-cli` / `pi-sdk`：继续下沉 startup pipeline 里剩余的 CLI 专属拼装逻辑
