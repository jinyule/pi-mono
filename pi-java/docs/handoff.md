...
- 阶段 8 `behavior parity`：已开始，已完成 session selector current/all scope toggle、sort toggle、named-only filter、path show/hide toggle、threaded sort 首版。
- 阶段 9：未开始。
...
1. 阶段 8：继续补 session selector 的 fuzzy sort / app-level keybinding 对齐等行为追平
